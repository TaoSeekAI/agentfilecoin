...
import { CONTRACT_ADDRESSES, TIME_CONSTANTS, TOKENS } from "../utils/index.js";
...
describe('PaymentsService', () => {
    ...
    describe('Service approvals', () => {
        ...
        it('should approve service as operator', async () => {
            ...
            const tx = await payments.approveService(serviceAddress, rateAllowance, lockupAllowance, TIME_CONSTANTS.EPOCHS_PER_MONTH);
            assert.exists(tx);
            assert.exists(tx.hash);
            assert.typeOf(tx.hash, 'string');
        });
        ...
        it('should throw for unsupported token in service operations', async () => {
            try {
                await payments.approveService(serviceAddress, 100n, 1000n, TIME_CONSTANTS.EPOCHS_PER_MONTH, 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
                assert.include(error.message, 'not supported');
    ...
    describe('Deposit and Withdraw', () => {
        ...
        it('should deposit with permit and approve operator', async () => {
            ...
            const tx = await payments.depositWithPermitAndApproveOperator(depositAmount, operator, rateAllowance, lockupAllowance, maxLockupPeriod);
            assert.exists(tx);
            assert.exists(tx.hash);
        });
