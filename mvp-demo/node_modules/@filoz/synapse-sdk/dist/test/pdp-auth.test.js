...
describe('Auth Signature Compatibility', () => {
    ...
    it('should generate CreateProofSet signature matching Solidity reference', async () => {
        const result = await authHelper.signCreateProofSet(FIXTURES.signatures.createProofSet.clientDataSetId, FIXTURES.signatures.createProofSet.payee, FIXTURES.signatures.createProofSet.withCDN);
        assert.strictEqual(result.signature, FIXTURES.signatures.createProofSet.signature, 'CreateProofSet signature should match Solidity reference');
    ...
    it('should generate AddRoots signature matching Solidity reference', async () => {
        const result = await authHelper.signAddRoots(FIXTURES.signatures.addRoots.clientDataSetId, FIXTURES.signatures.addRoots.firstAdded, ROOT_DATA);
        assert.strictEqual(result.signature, FIXTURES.signatures.addRoots.signature, 'AddRoots signature should match Solidity reference');
        const recoveredSigner = ethers.recoverAddress(result.signedData, result.signature);
        assert.strictEqual(recoveredSigner.toLowerCase(), FIXTURES.signerAddress.toLowerCase());
    });
    it('should generate ScheduleRemovals signature matching Solidity reference', async () => {
        const result = await authHelper.signScheduleRemovals(FIXTURES.signatures.scheduleRemovals.clientDataSetId, FIXTURES.signatures.scheduleRemovals.rootIds);
        assert.strictEqual(result.signature, FIXTURES.signatures.scheduleRemovals.signature, 'ScheduleRemovals signature should match Solidity reference');
        const recoveredSigner = ethers.recoverAddress(result.signedData, result.signature);
        assert.strictEqual(recoveredSigner.toLowerCase(), FIXTURES.signerAddress.toLowerCase());
    });
    it('should generate DeleteProofSet signature matching Solidity reference', async () => {
        const result = await authHelper.signDeleteProofSet(FIXTURES.signatures.deleteProofSet.clientDataSetId);
        assert.strictEqual(result.signature, FIXTURES.signatures.deleteProofSet.signature, 'DeleteProofSet signature should match Solidity reference');
        const recoveredSigner = ethers.recoverAddress(result.signedData, result.signature);
        assert.strictEqual(recoveredSigner.toLowerCase(), FIXTURES.signerAddress.toLowerCase());
    });
    it('should handle bigint values correctly', async () => {
        const result = await authHelper.signCreateProofSet(BigInt(12345), FIXTURES.signatures.createProofSet.payee, FIXTURES.signatures.createProofSet.withCDN);
    ...
    it('should generate consistent signatures', async () => {
        const sig1 = await authHelper.signCreateProofSet(FIXTURES.signatures.createProofSet.clientDataSetId, FIXTURES.signatures.createProofSet.payee, FIXTURES.signatures.createProofSet.withCDN);
        const sig2 = await authHelper.signCreateProofSet(FIXTURES.signatures.createProofSet.clientDataSetId, FIXTURES.signatures.createProofSet.payee, FIXTURES.signatures.createProofSet.withCDN);
    ...
    it('should handle empty root data array', async () => {
        const result = await authHelper.signAddRoots(FIXTURES.signatures.addRoots.clientDataSetId, FIXTURES.signatures.addRoots.firstAdded, []);
